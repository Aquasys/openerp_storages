...
class TestS3Interface(TransactionCase):
	...
	self.registry('res.partner').create(cr, uid, \
		 {'name': 'test_per_class_teardown_partner'})
	self.registry('res.partner').create(cr, uid, \
		{'name': 'test_per_class_teardown_partner_123'})
	# 2.Search for created parnter Records	
	res_id1 = self.registry('res.partner').search(cr, uid, \
			[('name', '=', 'test_per_class_teardown_partner')])
        self.assertEqual(1, len(res_id1), "Test partner1 not found.")
	res_id2 = self.registry('res.partner').search(cr, uid, \
		[('name', '=', 'test_per_class_teardown_partner_123')])
	...
	fname = FILE.split('/')[-1]
	# 3.creating record with file data for newly created parnters in ir_attachement
	# Attaching same file to both newly created Partner Records 
        attachment_id1 = self.attachment_model.create(cr, uid,{
             'name': fname, 
            'db_datas': data,
            'datas_fname': fname,
	...
	attachment_id2 = self.attachment_model.create(cr, uid,{
             'name': fname, 
            'db_datas': data,
            'datas_fname': fname,
            'res_model' : 'res.partner',
	...
	# 4. Check: 2 reacords are created for same file attached to different partners in ir_attachment
	att_id1 = self.attachment_model.search(cr, uid,\
		[('res_id', '=', res_id1[0]),('res_model', '=', 'res.partner'), \
			('name','=',fname)])
        self.assertEqual(1, len(att_id1), "Test attachement1 Notefound.")
	att_id2 = self.attachment_model.search(cr, uid,\
		 [('res_id', '=', res_id2[0]), ('res_model', '=', 'res.partner'), \
			('name','=',fname)])
	...
	look_id1 = self.lookup_model.search(cr, uid, [('res_id', '=', att_id1[0]),\
	 ('model_id', '=', 'ir_attachment')])
        self.assertEqual(1, len(look_id1), "Test Lookup Notefound.")
	look_id2 = self.lookup_model.search(cr, uid, [('res_id', '=', att_id2[0]),\
	 ('model_id', '=', 'ir_attachment')])
        self.assertEqual(1, len(look_id2), "Test Lookup Not found.")	
	# 6. Check for unique file name	
	look_data = self.lookup_model.read(cr, uid, look_id1, ['en_file_name'])[0]
	look_data1 = self.lookup_model.read(cr, uid, look_id2, ['en_file_name'])[0]
